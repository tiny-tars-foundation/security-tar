...
// Runs the same contract suite the D1 adapter satisfies (in a real adopter, via Miniflare)
// against these plain in-memory stores instead. Two structurally unrelated adapters passing
// identical assertions is the actual evidence "storage-agnostic" holds, not just an assertion
// made by interface shape (see ARCHITECTURE.md's "## Adapters" section).
